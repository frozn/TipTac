...
option = { type = "Check", var = "castBar", label = "Show Cast Bar", tip = "Will show a cast bar of the unit.", enabled = function(factory) return factory:GetConfigValue("enableBars") end };
if (LibFroznFunctions.hasWoWFlavor.castBarCastTimeAndNotInterruptibleNotAvailableForNonThePlayerUnitsWhichAreInCombat) then
	option.tip = option.tip .. "\nNOTE: Cast Time and Not-Interruptible Shield not available for non-the-player units which are in combat.";
end
tinsert(ttOptionsBars, option);

tinsert(ttOptionsBars, { type = "Check", var = "castBarAlwaysShow", label = "Always Show Cast Bar", tip = "Check this to always show the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end, x = 130 });
tinsert(ttOptionsBars, { type = "Color", var = "castBarCastingColor", label = "Cast Bar Casting Color", tip = "The casting color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end, y = 10 });
tinsert(ttOptionsBars, { type = "Color", var = "castBarChannelingColor", label = "Cast Bar Channeling Color", tip = "The channeling color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end });
tinsert(ttOptionsBars, { type = "Color", var = "castBarChargingColor", label = "Cast Bar Charging Color", tip = "The charging color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end });
tinsert(ttOptionsBars, { type = "Color", var = "castBarCompleteColor", label = "Cast Bar Complete Color", tip = "The complete color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end });
tinsert(ttOptionsBars, { type = "Color", var = "castBarFailColor", label = "Cast Bar Fail Color", tip = "The fail color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end });
tinsert(ttOptionsBars, { type = "Color", var = "castBarSparkColor", label = "Cast Bar Spark Color", tip = "The spark color of the cast bar", enabled = function(factory) return factory:GetConfigValue("enableBars") and factory:GetConfigValue("castBar") end });
...
option = { type = "Check", var = "showDebuffs", label = "Show Unit Debuffs", tip = "Show debuffs of the unit", enabled = function(factory) return factory:GetConfigValue("enableAuras") end };
if (LibFroznFunctions.hasWoWFlavor.aurasCooldownCountAndDebuffTypeNotAvailableInCombat) then
	option.tip = option.tip .. ".\nNOTE: Debuff Type not available in combat.";
end
tinsert(ttOptionsAuras, option);

tinsert(ttOptionsAuras, { type = "Check", var = "selfAurasOnly", label = "Only Show Auras Coming from You", tip = "This will filter out and only display auras you cast yourself.\nNOTE: Not available in combat. In this case auras are not filtered", enabled = function(factory) return factory:GetConfigValue("enableAuras") and (factory:GetConfigValue("showBuffs") or factory:GetConfigValue("showDebuffs")) end, y = 10 });
...
option = { type = "Check", var = "auraStackCount", label = "Show Amount of Stacks", tip = "With this option on, you will see the amount of stacks", enabled = function(factory) return factory:GetConfigValue("enableAuras") and (factory:GetConfigValue("showBuffs") or factory:GetConfigValue("showDebuffs")) end };
if (LibFroznFunctions.hasWoWFlavor.aurasCooldownCountAndDebuffTypeNotAvailableInCombat) then
	option.tip = option.tip .. ".\nNOTE: Not available in combat.";
end
tinsert(ttOptionsAuras, option);
...
if (TipTacTalents) then
	...
	tinsert(tttOptions, { type = "DropDown", var = "t_gearScoreAlgorithm", label = "GearScore Algorithm", list = { ["TacoTip"] = { value = LFF_GEAR_SCORE_ALGORITHM.TacoTip, tip = "The de-facto standard algorithm from addon TacoTip" }, ["TipTac"] = { value = LFF_GEAR_SCORE_ALGORITHM.TipTac, tip = PARENT_MOD_NAME .. "'s own implementation to simply calculate the GearScore is used here. This is the sum of all item levels weighted by performance per item level above/below base level of first tier set of current expansion, inventory type and item quality. Inventory slots for shirt, tabard and ranged are excluded." },}, tip = "Switch between different GearScore implementations", enabled = function(factory) return factory:GetConfigValue("t_enable") and factory:GetConfigValue("t_showGearScore") end });
...
if (TipTacItemRef) then
	...
	if (LibFroznFunctions.hasWoWFlavor.ShoppingTooltipHasCompareHeader) then
		tinsert(ttifOptions, { type = "Header", label = "Consider Shopping Tooltip Compare Header", tip = "Settings about shopping tooltip compare header", enabled = function(factory) return factory:GetConfigValue("if_enable") end });
		
		tinsert(ttifOptions, { type = "DropDown", var = "if_modifyShoppingTooltipCH", label = "Modify Compare Header", list = { ["|cffffa0a0Do nothing"] = "doNothing", ["Always hide Compare Header"] = "alwaysHideCH", ["Hide if icon is shown"] = { value = "hideCHIfIcon", disabled = function(factory) return not factory:GetConfigValue("if_showIcon") end } }, enabled = function(factory) return factory:GetConfigValue("if_enable") end });
		tinsert(ttifOptions, { type = "Check", var = "if_modifyIconOffsetForCH", label = "Modify Icon X/Y Offset", tip = "Modify the X/Y offset of the icon to prevent overlapping with the shopping tooltip compare header", enabled = function(factory) return factory:GetConfigValue("if_enable") and factory:GetConfigValue("if_showIcon") end });
		tinsert(ttifOptions, { type = "Slider", var = "if_iconOffsetX_CH", label = "Icon X Offset", min = -200, max = 200, step = 0.5, enabled = function(factory) return factory:GetConfigValue("if_enable") and factory:GetConfigValue("if_showIcon") and factory:GetConfigValue("if_modifyIconOffsetForCH") end });
		tinsert(ttifOptions, { type = "Check", var = "if_iconOffsetX_CH_addCHWidth", label = "Add Width of Compare Header", tip = "Dynamically add the width of the shopping tooltip compare header", enabled = function(factory) return factory:GetConfigValue("if_enable") and factory:GetConfigValue("if_showIcon") and factory:GetConfigValue("if_modifyIconOffsetForCH") end });
		tinsert(ttifOptions, { type = "Slider", var = "if_iconOffsetY_CH", label = "Icon Y Offset", min = -200, max = 200, step = 0.5, enabled = function(factory) return factory:GetConfigValue("if_enable") and factory:GetConfigValue("if_showIcon") and factory:GetConfigValue("if_modifyIconOffsetForCH") end });
	end
